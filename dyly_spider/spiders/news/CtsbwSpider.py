...
class CtsbwSpider(NewsSpider):
    ...
    def parse(self, response):
        data_list =response.xpath('//ul[@class="content news-photo picbig"]//article[@class="col-xs-12 list-single-article"]')
        if data_list is not None and len(data_list):
            for data in data_list:
                #  url
                url = data.xpath('//div[@class="col-xs-12 every-day-article-title"]/div/div/div/h4/a/@href').get().strip()
                out_id = url[29:-5]
                yield Request(
                    url,
                    meta={
