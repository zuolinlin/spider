...
def get_browser():
    ...
    if browser is None:
        ...
        browser = webdriver.Chrome(executable_path=r'spider/file/chromedriver.exe',
...
class DylySpiderDownloaderMiddleware(object):
    # Not all methods need to be defined. If a method is not defined,
...
class SeleniumExtMiddleware(object):
    def process_request(self, request, spider):
        if request.meta.get("selenium"):
            ...
            bws.get(request.url)
            spider.log("selenium 渲染url======> {0}".format(request.url))
            return HtmlResponse(url=bws.current_url, body=bws.page_source,
                                encoding="utf-8", request=request)
