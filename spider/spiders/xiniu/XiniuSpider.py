from spider.items import XiniuInstitutionItem, XiniuInvestmentEvents, XiniuInstitudynamic, XiniuNews, XiniuFun, \
...
class XiniuSpider(BaseSpider):
    ...
    #     self.driver.get("https://vip.xiniudata.com/org/investor/")
    #     time.sleep(2)
    #     size =os.path.getsize("spider/file/xiniudata")
    #     print(size)
    #     if size == 0:
    #         # 定义一个变量用与统计
    #         self.count = 0
    ...
    #         li_list = self.driver.find_elements_by_xpath(
    #             "//html/body/div/div/div[3]/div/div/div[2]/div/div[2]/section[1]/div/div/div/div/div[2]/div")
    #         file = open('spider/file/xiniudata', 'a')
    #         # 遍历读取写入文件
    #         for li in li_list:
    ...
    #     else:
    #         cookies = self.driver.get_cookies()
    #         error_file = open('spider/file/xiniudataerror', 'a')
    #         file = open("spider/file/xiniudata")
    #         for line in file:
    #             try:
    #                 # 打开链接，加载数据
    #                 self.driver.get(line)
